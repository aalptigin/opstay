...
export async function POST(request: NextRequest) {
    try {
        ...
        // Determine personId
        let personId = session.user.id;
        let personName = session.user.name;

        // Allow admin to create for others
        if (parsed.data.personId && session.user.role !== "STAFF") {
        ...
        const req = createLeaveRequest({
            ...
            personId,
            personName,
            unitId: session.user.unitId, // Or target user's unit
            unitName: "Birim",
