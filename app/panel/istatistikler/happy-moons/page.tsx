...
function isBlacklistRow(r: Row) {
  // status/type/kind gibi alanlar
  const status = s(pick(r, ["status", "state", "type", "record_type", "kind", "category"])).toLowerCase();
...
export default function Page() {
  ...
  const stats = useMemo(() => {
    ...
    for (const r of records) {
      ...
      // record’larda tarih farklı gelebilir
      const d =
        normalizeDate(pick(r, ["date", "gun_ay_yil", "created_date", "created_at_date", "created_at"])) ||
        "";
      if (!d) continue;
